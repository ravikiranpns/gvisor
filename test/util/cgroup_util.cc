...
PosixError Cgroup::PollControlFileForChange(absl::string_view name,
  ...
  ASSIGN_OR_RETURN_ERRNO(const int64 initial_value,
                         ReadIntegerControlFile(name));

  while (true) {
    ASSIGN_OR_RETURN_ERRNO(const int64 current_value,
                           ReadIntegerControlFile(name));
...
PosixErrorOr<Cgroup> Mounter::MountCgroupfs(std::string mopts) {
  ...
  std::cerr << absl::StreamFormat(
                   ...
                   mountpath, mopts)
            << std::endl;
  Cgroup cg = Cgroup(mountpath, mountpath);
...
bool CgroupsEntry::subsys_name_contains(absl::string_view query) const {
  const std::vector<std::string> names =
      StrSplit(subsys_name, absl::ByChar(','), absl::SkipEmpty());
